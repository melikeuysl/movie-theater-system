    ...
    <div className='px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden'>
      ...
      <div className='group grid grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto'>
        {dummyTrailers.map((trailer)=>(
            ...
            duration-300 transition max-md:h-60 md:max-60 cursor pointer'
            onClick={()=>setCurrentTrailer(trailer)}>
                <img src={trailer.image} alt="trailer" className='rounded-lg w-full h-full object-cover brightness-75' />
                <PlayCircle strokeWidth={.6} className="absolute top-1/2 left-1/2 
                w-5 md.w-8 h-5 md:h12 transform -translate-x-1/2
                -translate-y-y1/2" />
            </div>
